...
|---|---|---|---|---|
| Base model | Mistral 7B v0.3 | Mistral 7B v0.3 | Mistral 7B v0.3 | Mistral 7B v0.3 |
| Fine-tuned | no | yes | yes | yes |
| Training examples | - | 517 | 206 | 206 |
| Training source | - | mixed (PyMuPDF + Aryn) | PyMuPDF pages only | PyMuPDF pages only |
| Distractors (train) | - | 4 per example | 0 | 4 random pages |
| Epochs | - | 3 | 10 | 10 (crashed at 4.5, resumed) |
| Final loss | - | 0.985 | ~0.89 (epoch 2) | ~0.6 (messy) |
| n-chunks (test) | 20 | 5 | 1 (target page) | 1 or 5 |
| max_tokens | none | 50 | 50 | 50 |
| Test records | 108 | 108 | 60 (val) + 266 (full) | 60 (val only) |
| Val clean | - | - | 67% | 67% |
| Val adversarial | - | - | 63% | 57% |
| Full clean | - | - | 85% | - |
| Full adversarial | - | - | 67% | - |

## Key takeaways so far
1. **Fine-tuning works**: 49% → 67% on held-out clean data (V3 vs base)
2. **Distractors hurt at test time**: 67% → 63% on held-out data (V3 clean vs adversarial)
3. **Distractors in training didn't help**: V4 same or worse than V3 (but V4 training was messy)
4. **Overfitting present**: 85% full vs 67% val on clean data — more training data likely needed
5. **Biggest levers to try next**: more training data (686 candidates available, only used 103), clean distractor re-run, more epochs on clean data
